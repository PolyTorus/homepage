  ...
  return function (renderChildRef) {
    return function (document) {
      ...
      var buildWidget = function (spec) {
            ...
            return function (st, slot) {
              if (st instanceof Data_Maybe.Just) {
                ...
                if (slot instanceof Halogen_Component.ThunkSlot) {
                  var step$prime = Halogen_VDom_Machine.step(
                    st.value0,
                    slot.value0
                  );
                  return Halogen_VDom_Machine.mkStep(
                    new Halogen_VDom_Machine.Step(
                      Halogen_VDom_Machine.extract(step$prime),
                      new Data_Maybe.Just(step$prime),
                      $lazy_patch(103),
                      done
                    )
                  );
          ...
          function () {
            return function (slot) {
              ...
              if (slot instanceof Halogen_Component.ThunkSlot) {
                ...
                return Halogen_VDom_Machine.mkStep(
                  new Halogen_VDom_Machine.Step(
                    Halogen_VDom_Machine.extract(step),
                    new Data_Maybe.Just(step),
                    $lazy_patch(89),
                    done
                  )
                );
        ...
        var $lazy_renderComponentSlot = $runtime_lazy(
          "renderComponentSlot",
          "Halogen.VDom.Driver",
          function () {
            return function (cs) {
          ...
          return function (v1) {
            if (v1 instanceof Data_Maybe.Nothing) {
              return function __do() {
                ...
                $$void(
                  Web_DOM_Node.appendChild(node)(
                    Web_HTML_HTMLElement.toNode(container)
                  )
                )();
                return {
                  machine: machine,
            ...
            if (v1 instanceof Data_Maybe.Just) {
              return function __do() {
                ...
                var machine$prime = Halogen_VDom_Machine.step(
                  v1.value0.machine,
                  v
                );
                var newNode = Halogen_VDom_Machine.extract(machine$prime);
                when(
                  not(Unsafe_Reference.unsafeRefEq)(v1.value0.node)(newNode)
...
var runUI = function (component) {
  return function (i) {
    return function (element) {
      return bind1(
        liftEffect(
          map(Web_HTML_HTMLDocument.toDocument)(
            bindFlipped(Web_HTML_Window.document)(Web_HTML.window)
          )
        )
      )(function (document) {
        return Halogen_Aff_Driver.runUI(renderSpec(document)(element))(
          component
        )(i);
      });
    };
